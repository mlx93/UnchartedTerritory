...
### Purpose
Extend Chartsmith with an AI-powered Chart Validation Agent that provides real-time validation feedback, plus enhance the provider switching experience to support live mid-conversation model changes.
...
### Scope
This PR delivers the Chart Validation Agent feature and completes the provider switching UX to Cursor-style live switching, fulfilling the Uncharted Territory Challenge requirements.
...
4. **Provider lock-in per session**: Must start new chat to switch models
...
**Acceptance Criteria**:
- Provider dropdown visible at all times (Cursor-style)
...
- Validates current chart in context
...
- Formatted summary card in chat
...
**Location**: Inline in chat as assistant message
...
- Severity icons: ❌ critical, ⚠️ warning, ℹ️ info, ✅ pass

### Live Provider Switcher

**Location**: Chat header, always visible

**Layout**:
```
┌─────────────────────────────────────────────────────────┐
│ Chartsmith Chat              [GPT-4o ▼] [⋮]            │
├─────────────────────────────────────────────────────────┤
│                    ┌─────────────────┐                  │
│                    │ ✓ GPT-4o        │                  │
│                    │   Claude 4.5    │                  │
...
**Endpoint**: `POST /api/chat` (modified from PR1)

**Additional Request Fields**:
```
{
  ...existing fields...,
  "provider": "anthropic",  // Can change mid-conversation
...
- [ ] Tests cover tool invocation and result parsing
...
- Go backend for validation endpoint
- Existing chart file system access
- AI SDK tool calling (from PR1)
...
Post-PR2 features for consideration:
...
*Document End - PRD3: PR2 Functional Requirements*
